...
# 이진 분류를 위한 amEmotion_binary 열 생성
# 1 (Very unpleasant), 2 (Unpleasant), 3 (Moderate), 4 (Pleasant), 5 (Very pleasant)
merged_df['amEmotion_binary'] = merged_df['amEmotion'].apply(lambda x: 1 if x >= 3 else 0)
merged_df.drop(['user', 'num'], axis=1, inplace=True)

# 특성과 타겟 분리
X = merged_df.drop(['amEmotion', 'amEmotion_binary'], axis=1)
y = merged_df['amEmotion_binary']
